...
const NewBooking = () => {
  ...
  const handleDateChange  = (item) => setDates([{ startDate: item.selection.startDate, endDate: item.selection.endDate, key: "selection" }]);
  const handleRoomSelect  = (e) => {
    const selected = rooms.find((r) => r._id === e.target.value);
    setRoom(selected); setSelectedRooms([]); setSelectedRoomNumbers([]);
  };
  const updateQuantity    = (field, value) => setOptions((prev) => ({ ...prev, [field]: Math.min(Math.max(prev[field] + value, 0), 5) }));
  ...
  const isRoomAvailable = (roomNumber) => {
    const startTime = dates[0].startDate.getTime(), endTime = dates[0].endDate.getTime();
  ...
  const handleBookingSubmit = async (e) => {
    ...
    const bookingData = {
      ...info,
      roomTitle: room.title, adults: options.adults, children: options.children,
      startDate: dates[0].startDate, endDate: dates[0].endDate,
      numberOfRooms: options.rooms, selectedRooms, roomNumbers: selectedRoomNumbers,
      price: info.amount || totalPrice, checkedIn,
      registeredBy: `${user.firstName} ${user.lastName}`,
    };
    try {
      await Promise.all(selectedRooms.map((roomId) =>
        api.put(`/rooms/availability/${roomId}`, { dates: getDatesInRange(dates[0].startDate, dates[0].endDate) })
      ));
    ...
    <div style={{
      ...
      <div style={{ display: "flex", gap: "2rem", flexWrap: "wrap", alignItems: "flex-start" }}>
        ...
        >
              ...
              {[
                ...
                <button
                  ...
                  style={{
                    ...
                    border: `1px solid ${datePickerOpen ? currentColor : c.border}`,
                    background: c.inputBg, color: c.text,
                ...
                >
                  <span style={{ fontSize: "9px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: c.muted }}>
                    {label}
                  </span>
          ...
          {/* Adults / Children / Rooms counters */}
            ...
            {["adults", "children", "rooms"].map((field) => (
              <div key={field} style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  ...
                  <span style={{ fontSize: "15px", fontWeight: 600, color: c.text, minWidth: "20px", textAlign: "center" }}>
                    {options[field]}
                  </span>
                  <button
                    type="button"
                  ...
                  return (
                    <label
                      ...
                      style={{
                        ...
                        padding: "5px 12px", borderRadius: "8px", cursor: available ? "pointer" : "not-allowed",
                        fontSize: "12px", fontWeight: 600,
                        border: `1px solid ${selected ? currentColor : c.border}`,
                        background: selected ? `${currentColor}15` : c.surface,
                        color: !available ? c.muted : selected ? currentColor : c.text,
          ...
          <div style={{
            ...
            <button
              ...
              style={{
                ...
                background: checkedIn ? currentColor : isDark ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.15)",
